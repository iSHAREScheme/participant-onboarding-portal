...
type Proposal struct {
	...
	// IdCheckMethod records how the applicant proved who they are:
	// "eherkenning", "eidas" or "vc" (a verified presentation).
	IdCheckMethod string `json:"idCheckMethod"`
	// Verifiable-credential evidence, set when the proposal was pre-filled from
	// a presentation. It is kept so an operator reviewing the proposal can see
	// exactly which credential asserted which field, and so the auto-accept rule
	// has something auditable to key on.
	VcVerified        bool       `json:"vcVerified"`
	VcVerifiedAt      *time.Time `json:"vcVerifiedAt,omitempty"`
	VcHolder          string     `json:"vcHolder,omitempty"`
	VcCredentialTypes string     `json:"vcCredentialTypes,omitempty"`
	VcIssuers         string     `json:"vcIssuers,omitempty"`
	// VcPrefill is the marshalled {field: value} / {field: credentialType} pair
	// the verifier produced, shown on the admin review screen.
	VcPrefill string `json:"vcPrefill,omitempty" gorm:"type:text"`
}
